...
CREATE TABLE IF NOT EXISTS HistorialCostos (id_historial_costos integer, estatus boolean, fecha_creacion timestamp, fecha_modificacion timestamp, usuario_creacion varchar(255), usuario_modificacion varchar(255), costo_anterior numeric(12,2), costo_nuevo numeric(12,2) not null, diferencia numeric(12,2), motivo varchar(500), referencia varchar(100), usuario varchar(100), fecha_cambio timestamp not null, id_empresa integer not null, id_producto integer not null, id_proveedor integer not null, primary key (id_historial_costos));
CREATE INDEX IF NOT EXISTS idx_historial_costos_prov_prod_emp_fecha ON HistorialCostos(id_proveedor, id_producto, id_empresa, fecha_cambio DESC);
...
CREATE INDEX IF NOT EXISTS idx_proveedor_producto_proveedor_empresa ON ProveedorProducto(id_proveedor, id_empresa);
CREATE INDEX IF NOT EXISTS idx_proveedor_producto_prov_prod_emp ON ProveedorProducto(id_proveedor, id_producto, id_empresa);

CREATE TABLE IF NOT EXISTS Proveedores (id_proveedor integer, estatus boolean, fecha_creacion timestamp, fecha_modificacion timestamp, usuario_creacion varchar(255), usuario_modificacion varchar(255), contacto varchar(255), correo varchar(255), direccion varchar(255), nombre varchar(255) not null, telefono varchar(255), razon_social varchar(255), rfc varchar(255), tipo_proveedor varchar(255) not null default 'PROVEEDOR_INFORMAL', categoria_principal varchar(255), estado_proveedor varchar(255) not null default 'ACTIVO', estado_proveedor_anterior varchar(255), ultima_accion_estado varchar(255), motivo_cambio_estado varchar(500), usuario_cambio_estado varchar(255), fecha_cambio_estado timestamp, notas_internas varchar(2000), modalidad_abastecimiento varchar(255) not null default 'ENTREGA_DOMICILIO', pedido_whatsapp boolean, pedido_llamada boolean, pedido_app boolean, visita_ruta boolean, compra_mostrador boolean, dias_visita_entrega varchar(255), horario_habitual varchar(255), pedido_minimo numeric(12,2), tiempo_estimado_entrega varchar(255), costo_envio numeric(12,2), observaciones_abastecimiento varchar(2000), forma_pago_principal varchar(255) not null default 'CONTADO', maneja_credito boolean, dias_credito integer, limite_credito numeric(12,2), permite_devoluciones boolean, cambios_caducidad boolean, bonificaciones boolean, descuentos_frecuentes boolean, notas_comerciales varchar(2000), id_empresa integer not null, primary key (id_proveedor));
...
CREATE INDEX IF NOT EXISTS idx_proveedor_activo_proveedor_empresa ON ProveedorActivo(id_proveedor, id_empresa);
...
CREATE INDEX IF NOT EXISTS idx_proveedor_activo_hist_activo_empresa ON ProveedorActivoHistorial(id_proveedor_activo, id_empresa);
