  ...
  const rowExpansionTemplate = (data) => (
    <div className="p-3">
      <DataTable value={data.lotes} size="small" className="inv-lotes-table" responsive>
        ...
        <Column field="categories" header="Cantidad" body={(l) => <span className="tabular">{l.cantidad}</span>} />
    ...
    <Shell>
      <div className="inv-page">
        ...
        <div className="inv-header-bar">
          <div className="inv-header-title"><i className="pi pi-box" /> INVENTARIO GLOBAL</div>
        ...
        <div className="inv-toolbar-glass">
          <span className="p-input-icon-left inv-search-wrapper">
            ...
            <InputText value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar por nombre, SKU, categoría o proveedor..." className="inv-search-input" />
          </span>
          <div className="inv-toolbar-actions">
            <Button icon="pi pi-filter" label="Filtros" className="inv-btn inv-btn--ghost" onClick={() => setFilterOpen(true)} />
            <Button icon="pi pi-plus" label="Nuevo Producto" className="inv-btn inv-btn--primary" onClick={() => setModalAgregar(true)} />
          </div>
        </div>
        ...
        <DataTable
          ...
          onKeyDown={(e) => {
            if (e.key === 'Enter') {
              e.preventDefault();
              const rowElement = document.querySelector('.p-datatable-tbody > tr.p-highlight');
              if (rowElement) {
                const firstBtn = rowElement.querySelector('.inv-action-btn');
        ...
        >
          ...
          <Column header="Imagen" body={imagenBody} />
          
          {/* COLUMNA NOMBRE - TOOLTIP PREMIUM REDUCIDO */}
          <Column 
            field="nombre" 
            header="Nombre" 
            sortable 
            ...
            body={(row) => row.nombre}
            tooltip={(row) => row.nombre}
            tooltipOptions={{ position: "top", className: "inv-tooltip-cell", showDelay: 150, hideDelay: 50 }}
          />
          
          <Column field="sku" header="SKU" />
          
          {/* COLUMNA PROVEEDOR - TOOLTIP PREMIUM REDUCIDO */}
          <Column 
            field="proveedor" 
            header="Proveedor" 
            sortable 
            className="text-truncate" 
            body={(row) => row.proveedor}
            tooltip={(row) => row.proveedor}
            tooltipOptions={{ position: "top", className: "inv-tooltip-cell", showDelay: 150, hideDelay: 50 }}
          />
          
          <Column field="stock" header="Stock" sortable body={stockBody} />
          <Column field="estadoStock" header="Estado" sortable 
            sortFunction={(e) => {
              const PESOS = { "AGOTADO": 1, "CRITICO": 2, "BAJO": 3, "OPTIMO": 4 };
              e.data.sort((a, b) => (PESOS[getStockEstado(a)] - PESOS[getStockEstado(b)]) * e.order);
            }}
            body={(row) => <Tag value={getStockEstado(row)} severity={getStockEstado(row) === "OPTIMO" ? "success" : "danger"} rounded />}
          />
          ...
          <Column header="Acciones" body={accionesBody} frozen />
        </DataTable>
        ...
        <ModalEditarProducto open={modalEditar.open} producto={modalEditar.producto} onHide={() => setModalEditar({ open: false, producto: null })} onSave={(p) => { upsertProductoLocal(p, modalEditar.producto?.lotes || []); setModalEditar({ open: false, producto: null }); }} categoriasOptions={categoriasOptions} proveedoresOptions={proveedoresOptions} unidadesOptions={unidadesOptions} almacenesOptions={almacenesOptions} />
        <ModalAjusteStock open={modalAjuste.open} producto={modalAjuste.producto} onHide={() => setModalAjuste({ open: false, producto: null })} onApply={(p, meta) => { handleAjusteStock(p, meta); setModalAjuste({ open: false, producto: null }); }} almacenesOptions={almacenesOptions} />
        <ModalAlertasStock open={modalAlertas.open} producto={modalAlertas.producto} onHide={() => setModalAlertas({ open: false, producto: null })} onSave={(p) => { handleConfigurarUmbrales(p); setModalAlertas({ open: false, producto: null }); }} />
        <ModalEliminarProducto open={modalEliminar.open} producto={modalEliminar.producto} onHide={() => setModalEliminar({ open: false, producto: null })} onConfirm={(id) => { eliminarProductoLocal(id); setModalEliminar({ open: false, producto: null }); }} />
        <ModalAgregarProducto open={modalAgregar} onHide={() => setModalAgregar(false)} onCreate={handleCreateProducto} categoriasOptions={categoriasOptions} proveedoresOptions={proveedoresOptions} almacenesOptions={almacenesOptions} unidadesOptions={unidadesOptions} />
      </div>
    </Shell>
  );
}